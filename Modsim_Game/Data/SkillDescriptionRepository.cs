...
namespace Modsim_Game.Data
{
    public static class SkillDescriptionRepository
    {
        ...
        static SkillDescriptionRepository()
        {
            ...
            Add("First Aid", "Active", "n_firstAid.png", "Heal yourself for 5 HP. Not a crazy powerful skill, but mages seem to like it for saving money on healing items.", null, (lv, hp, sp) => new List<SkillEffect> { new SkillEffect { Label = "HP Restored", Value = "5" } });
            ...
            Add("Fire Ball", "Offensive, Magic", "mg_fireBall.png", "Hits every enemy in a 5x5 area around the target with an MATK of (70+10*SkillLV)% and Fire Element.", null, (lv, hp, sp) => new List<SkillEffect> { new SkillEffect { Label = "MATK Damage", Value = $"{70 + 10 * lv}%" } });
            Add("Fire Wall", "Offensive, Magic", "mg_fireWall.png", "Creates 3 cells of the Fire Wall effect. Each cell can deliver up to 4+SkillLV Fire Element hits at MATK*0.5.", null, (lv, hp, sp) => new List<SkillEffect> {
            ...
            Add("Attention Concentrate", "Active, Physical", "ac_attentionConcentrate.png", "Increases DEX and AGI of the casting character by (2+1*SkillLV)%. Detects hidden characters in 3 cells range.", null, (lv, hp, sp) => new List<SkillEffect> {
            ...
            Add("Angelus", "Active", "al_angelus.png", "Increases the DEF from VIT of all party members on screen by (5*SkillLV)%.", null, (lv, hp, sp) => new List<SkillEffect> { new SkillEffect { Label = "VIT DEF Bonus", Value = $"+{5 * lv}%" } });
            Add("Blessing", "Support / Buff, Magic", "al_blessing.png", "Increases STR, DEX and INT of target. Halves stats of Undead/Demon monsters.", null, (lv, hp, sp) => new List<SkillEffect> { new SkillEffect { Label = "Stat Bonus", Value = $"+{lv}" } });
            ...
            Add("Cart Revolution", "Offensive, Physical", "mc_cartRevolution.png", "Does ATK*150% neutral-property damage to 3x3 area. Pushes back 2 cells.", null, (lv, hp, sp) => new List<SkillEffect> { new SkillEffect { Label = "Damage ATK", Value = "150%" }, new SkillEffect { Label = "Knockback", Value = "2 cells" } });
            ...
            Add("Sprinkle Sand", "Offensive, Physical", "tf_sprinkleSand.png", "130% ATK damage with a 20% chance to cause blind effect. Earth property.", null, (lv, hp, sp) => new List<SkillEffect> { new SkillEffect { Label = "Blind Chance", Value = "20%" }, new SkillEffect { Label = "Damage ATK", Value = "130%" } });
            Add("Back Sliding", "Active, Physical", "tf_backSliding.png", "Moves you backwards 5 cells instantly.", null, (lv, hp, sp) => new List<SkillEffect>());
            Add("Pick Stone", "Active", "tf_pickStone.png", "Adds one Stone item to your inventory.", null, (lv, hp, sp) => new List<SkillEffect>());
            Add("Throw Stone", "Offensive, Misc", "tf_throwStone.png", "Does 50 fixed damage and has a 3% chance of causing stun. Consumes 1 stone.", null, (lv, hp, sp) => new List<SkillEffect> { new SkillEffect { Label = "Fixed Damage", Value = "50" }, new SkillEffect { Label = "Stun Chance", Value = "3%" } });
            Add("Hiding", "Active", "tf_hiding.png", "Makes the character invisible to most players and monsters. Drains SP.", null, (lv, hp, sp) => new List<SkillEffect> { new SkillEffect { Label = "Duration", Value = $"{30 * lv}s" } });
            Add("Detoxify", "Support / Buff, Physical", "tf_detoxify.png", "Cures poison status on target.", null, (lv, hp, sp) => new List<SkillEffect>());
        }
